...
public class MigrationService : IMigrationService
{
  ...
  private static async Task EnsureAdministratorUser(IServiceProvider provider)
  {
    ...
    var user = await manager.FindByNameAsync(Constants.AdminMailAddress);
    if (user != null)
    {
      return;
    }
    ...
    {
      UserName = Constants.AdminMailAddress,
      Email = Constants.AdminMailAddress
    };
    ...
    await manager.AddToRoleAsync(applicationUser, Roles.Administrator);
  }
}
